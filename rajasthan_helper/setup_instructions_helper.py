#!/usr/bin/env python3
"""
Create .github directory and copilot-instructions.md for Rajasthan Helper project.
This script can be run from anywhere to set up the instruction file.
"""
...
def main():
    ...
    instructions_file = os.path.join(github_dir, 'copilot-instructions.md')
    
    # Create .github directory
    try:
        os.makedirs(github_dir, exist_ok=True)
        print(f"✓ Directory created/verified: {github_dir}")
...
The template `copilot-instructions.md` includes sections for:
...
'''
    ...
    try:
        with open(instructions_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✓ File created: {instructions_file}")
    except Exception as e:
    ...
    if os.path.exists(instructions_file):
        ...
        print(f"  Directory: {github_dir}")
        print(f"  File: {instructions_file}")
        return True
    else:
        print("✗ Error: File not found after creation")
        return False
